"""
QUANTA Thor Screener
====================
Thor is NOT a per-coin agent. It is a market-wide screener.
...
Architecture:
  ...
  - On every closed 5m bar → runs the Thor check inline
  - When Thor fires → pushes a ThorSignal onto an output queue
  - QUANTA_bot consumes the queue and enters the trade directly

Why separate from the main prediction pipeline:
  - Thor doesn't need features, ML, or multi-timeframe analysis
...
Signal flow:
  ThorScreener.signal_queue  →  Bot._thor_signal_consumer  →  opportunity dict  →  rl_opportunities
"""
...
try:
    ...
    sys.stdout = _old
    _ev = _cfg.events
    BODY_RATIO_MULT = _ev.thor_body_ratio_mult    # 15.0
    QUIET_BODY_PCT  = _ev.thor_quiet_body_pct     # 0.5  (as %)
    VOL_MULT        = _ev.thor_vol_mult            # 2.0
    BODY_MIN        = _ev.thor_body_min            # 0.6
    IMMEDIATE_BODY_RATIO_MULT = _ev.thor_immediate_body_ratio_mult
    IMMEDIATE_BODY_MIN        = _ev.thor_immediate_body_min
    IMMEDIATE_VOL_MULT        = _ev.thor_immediate_vol_mult
    CONTINUATION_VOL_MULT     = _ev.thor_continuation_vol_mult
    SCORE_BODY_RATIO_WEIGHT   = _ev.thor_score_body_ratio_weight
    SCORE_VOL_RATIO_WEIGHT    = _ev.thor_score_vol_ratio_weight
    SCORE_BODY_EFF_WEIGHT     = _ev.thor_score_body_eff_weight
    SCORE_QUIET_WEIGHT        = _ev.thor_score_quiet_weight
    SCORE_CONFIRM_WEIGHT      = _ev.thor_score_confirm_weight
    TIER_A_CONFIDENCE         = _ev.thor_tier_a_confidence
    TIER_B_CONFIDENCE         = _ev.thor_tier_b_confidence
    TIER_C_CONFIDENCE         = _ev.thor_tier_c_confidence
    TIER_A_SIZE_MULT          = _ev.thor_tier_a_size_mult
    TIER_B_SIZE_MULT          = _ev.thor_tier_b_size_mult
    TIER_C_SIZE_MULT          = _ev.thor_tier_c_size_mult
    TIER_B_BS_FLOOR           = _ev.thor_tier_b_bs_floor
    TIER_C_BS_FLOOR           = _ev.thor_tier_c_bs_floor
    TP_ATR          = _ev.thor_tp_atr
    SL_ATR          = _ev.thor_sl_atr
    MAX_BARS        = _ev.thor_max_bars
    BANK_ATR        = _ev.thor_bank_atr
    BANK_FRACTION   = _ev.thor_bank_fraction
    RUNNER_TRAIL_ATR= _ev.thor_runner_trail_atr
    MAX_BARS_PRE    = _ev.thor_max_bars_pre_bank
    MAX_BARS_POST   = _ev.thor_max_bars_post_bank
except Exception:
...
class ThorSignal:
    ...
    score:         float          # deterministic Thor quality score (0-100)
...
def _thor_score(meta: dict, confirm_strength: float) -> float:
...
def _tier_payload(tier: str, entry_mode: str, close: float, atr: float, meta: dict, confirm_strength: float) -> dict:
    ...
    return {
        ...
        'score': _thor_score(meta, confirm_strength),
...
def _thor_check(buf: _SymbolBuffer) -> Optional[dict]:
    """
    Run the Thor trigger on the most-recently closed bar.
...
class ThorScreener:
    """
    Market-wide Thor screener.

    Usage:
        screener = ThorScreener(symbols=all_symbols, on_signal=callback)
    ...
    def __init__(
        self,
        symbols: List[str],
        on_signal: Optional[Callable[[ThorSignal], None]] = None,
        proxy_url: Optional[str] = None,
    ):
        self.symbols      = list(symbols)
        self.on_signal    = on_signal           # optional callback; queue always filled
        self.proxy_url    = proxy_url
        self.signal_queue: queue.Queue[ThorSignal] = queue.Queue(maxsize=500)
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run_loop, name="ThorScreener", daemon=True
        )
        self._thread.start()
    ...
    async def _supervisor(self) -> None:
        ...
        # 5m only — we don't need other timeframes for the Thor check
...
# ── Backward-compatibility aliases (imports from quanta_nike_screener still work) ──
NikeSignal   = ThorSignal
NikeScreener = ThorScreener
_nike_check  = _thor_check
